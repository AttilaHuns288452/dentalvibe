...
const STEPS = ['Your Details', 'Contact', 'Account']
...
export default function Login() {
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-gray-50 px-4 pt-12 pb-8">
      <div className="bg-primary-600 text-white rounded-2xl pt-5 pb-4 px-4 text-center shadow-sm">
        <div className="w-14 h-14 mx-auto rounded-full bg-white text-primary-600 flex items-center justify-center">
        ...
        <form onSubmit={submit} className="mt-4">
          <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3 shadow-sm">
            <div className="flex rounded-lg bg-gray-100 p-1 text-sm font-medium">
              <button type="button" onClick={() => setMode('signin')} className="flex-1 h-11 rounded-md bg-white shadow flex items-center justify-center">Sign In</button>
              <button type="button" onClick={() => setMode('signup')} className="flex-1 h-11 rounded-md text-gray-500 flex items-center justify-center">Register</button>
            </div>
            <label className="block">
              <span className="text-xs font-medium text-gray-500">Email address</span>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required
                     className="mt-1.5 w-full h-11 border border-gray-200 rounded-lg px-3 text-sm" placeholder="you@email.com" />
            </label>
            <div className="block">
  ...
  return (
    <form onSubmit={submit} className="mt-4">
      <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-4 shadow-sm">
        <h2 className="text-sm font-bold text-gray-900 text-center">Create your account</h2>
        <div className="flex items-center justify-center gap-1.5">
          {STEPS.map((label, i) => (
            <div key={label} className="flex items-center gap-1.5">
              <div className={'w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-bold ' + (step === i + 1 ? 'bg-primary-700 text-white' : 'bg-primary-100 text-primary-700')}>{i + 1}</div>
          ...
          <>
            <div className="grid grid-cols-2 gap-3">
              <label className="block"><span className="text-xs font-medium text-gray-500">First name</span>
                <input value={f.first} onChange={set('first')} required className="mt-1.5 w-full h-11 border border-gray-200 rounded-lg px-3 text-sm" /></label>
              <label className="block"><span className="text-xs font-medium text-gray-500">Last name</span>
                <input value={f.last} onChange={set('last')} required className="mt-1.5 w-full h-11 border border-gray-200 rounded-lg px-3 text-sm" /></label>
            </div>
            <label className="block"><span className="text-xs font-medium text-gray-500">Home address</span>
              <input value={f.address} onChange={set('address')} className="mt-1.5 w-full h-11 border border-gray-200 rounded-lg px-3 text-sm" /></label>
            <label className="block"><span className="text-xs font-medium text-gray-500">Birthdate</span>
                <input type="date" value={f.birthdate} onChange={set('birthdate')} required className="mt-1.5 w-full h-11 border border-gray-200 rounded-lg px-3 text-sm" /></label>
            <div className="mt-3"><span className="text-xs font-medium text-gray-500">Sex</span>
                <div className="flex gap-2 mt-1.5">
                  {['Male', 'Female'].map((sx) => (
                    <button key={sx} type="button" onClick={() => setF((v) => ({ ...v, sex: sx }))}
                            className={'flex-1 h-11 rounded-lg border text-sm font-semibold flex items-center justify-center ' + (f.sex === sx ? 'bg-primary-50 text-primary-700 border-primary-300' : 'bg-white text-gray-600 border-gray-200')}>{sx}</button>
                  ))}
                </div>
              </div>
            <label className="block"><span className="text-xs font-medium text-gray-500">Medical Note</span>
              <textarea value={f.medical_note} onChange={set('medical_note')} rows={2} placeholder="Allergies in…"
                        className="mt-1.5 w-full border border-gray-200 rounded-lg px-3 py-2 text-sm resize-none" /></label>
          </>
        )}
        {step === 2 && (
          <>
            <label className="block"><span className="text-xs font-medium text-gray-500">Emergency contact</span>
              <input value={f.emergency_contact} onChange={set('emergency_contact')} placeholder="Name · +63 917 555 0000" required
                     className="mt-1.5 w-full h-11 border border-gray-200 rounded-lg px-3 text-sm" /></label>
        ...
        {step === 3 && (
          <>
            <label className="block"><span className="text-xs font-medium text-gray-500">Email address</span>
              <input type="email" value={f.email} onChange={set('email')} required className="mt-1.5 w-full h-11 border border-gray-200 rounded-lg px-3 text-sm" /></label>
            <div className="block">
              <label htmlFor="reg-pw" className="text-xs font-medium text-gray-500">Password</label>
              <span className="relative block">
                <input id="reg-pw" type={showPw ? 'text' : 'password'} value={f.password} onChange={set('password')} required className="mt-1.5 w-full h-11 border border-gray-200 rounded-lg px-3 pr-11 text-sm" />
                <button type="button" onClick={() => setShowPw(!showPw)} aria-label={showPw ? 'Hide password' : 'Show password'}
