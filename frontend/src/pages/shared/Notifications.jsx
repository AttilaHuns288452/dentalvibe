...
export default function Notifications({ roleBase = '' }) {
  ...
  const Row = ({ n }) => (
    <div className="flex gap-3 px-3.5 py-3 border-b border-gray-100 last:border-0">
      <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-none ${ICON_BG[n.icon] ?? ICON_BG.bell}`}>
        <svg viewBox="0 0 24 24" className="w-4.5 h-4.5 w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d={ICON_PATHS[n.icon] ?? ICON_PATHS.bell} /></svg>
      </div>
  ...
  return (
    <div className="px-4 py-4 space-y-3">
      ...
      <div className="bg-primary-50 border border-primary-100 rounded-lg px-3.5 py-2.5 text-[11px] text-primary-800 flex gap-2">
        <svg viewBox="0 0 24 24" className="w-4 h-4 flex-none mt-0.5" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d={ICON_PATHS.mail} /></svg>
        Every notification is also emailed to you — email-only delivery, no SMS.
      </div>
    </div>
  )
}
