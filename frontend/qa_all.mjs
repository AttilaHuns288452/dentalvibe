...
check('owner: pending banner', /booking request(s)? waiting/.test(await pg.locator('main').textContent()))
...
await pg.fill('input[type="date"]', '2026-10-10')
await pg.locator('button:has-text("Submit Booking Request")').click()
await pg.waitForTimeout(1400)
check('patient: booking confirmed', (await pg.locator('main h1').textContent()).includes('Booking Request Submitted'))
await pg.goto(BASE + '/appointments', { waitUntil: 'networkidle' })
await pg.waitForTimeout(1200)
check('patient: appointment listed', /pending/.test(await pg.locator('main').textContent()))
