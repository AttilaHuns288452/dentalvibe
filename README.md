...
- Scheduling capacity: capacity comes from **per-dentist work schedules** (`dentist_work_schedules`: weekday + working hours per dentist, Owner-configurable). Appointments are **automatically assigned** to an available dentist (least-loaded scheduled dentist, tie-break id), each dentist has **independent overlap protection**, and multiple dentists can handle **parallel appointments** at the same time (N scheduled dentists = N parallel resources — nothing is hardcoded). **Ready/Not Ready is a day-of operational presence state** (`dentist_ready`, scoped to the Manila clinic date) used for staffing visibility and conflict warnings — it does **not** control future booking capacity.
...
- A paid appointment holds its exact slot on **its assigned dentist** (per-dentist overlap guard + unique index); the same time on a **different** dentist is a legitimate parallel booking.
