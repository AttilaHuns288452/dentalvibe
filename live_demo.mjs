...
import('/home/attila/.hermes/hermes-agent/node_modules/playwright/index.mjs').then(async ({ chromium }) => {
  ...
  await po.locator('input:not([type="number"]):not([type="checkbox"])').last().fill('Demo supplies ' + Date.now())
  await po.locator('input[type="number"]').first().fill('150')
  await po.locator('select').first().selectOption({ label: 'Supplies' }).catch(() => {})
  await po.locator('button:has-text("Save Transaction")').click()
  await po.waitForTimeout(1500)
  ...
  const svcBox = po.locator('input').first()
  await svcBox.fill('Demo Cleaning')
  await po.locator('input[type="number"]').first().fill('999')
  await po.locator('button:has-text("Save")').last().click()
  await po.waitForTimeout(1500)
  await pg.goto(BASE + '/book', { waitUntil: 'networkidle' })
  ...
  t('S4 patient: completed visit in Past w/ Receipt', patTxt.includes('Completed') && patTxt.includes('Receipt'))
  await pd.goto(BASE + '/doctor/calendar', { waitUntil: 'networkidle' })
  ...
  const r1 = await M.from('appointments').insert({ patient_id: pidM, service_id: slotRow.service_id, requested_date: String(new Date()).slice(0, 10), scheduled_at: slotRow.scheduled_at, price: slotRow.price })
  const r1b = r1.error ? null : await M.rpc('fn_submit_payment_proof', { p_appointment: r1.data?.[0]?.id ?? (await M.from('appointments').select('id').eq('scheduled_at', slotRow.scheduled_at).eq('patient_id', pidM).limit(1)).data?.[0]?.id, p_image: 'x' })
  ...
  await M.from('appointments').delete().eq('patient_id', pidM).eq('payment_status', 'unpaid').eq('requested_date', String(new Date()).slice(0, 10))
